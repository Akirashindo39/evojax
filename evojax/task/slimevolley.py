...
class Particle:
    ...
    def display(self, canvas):
        # Handle arrays by extracting the first element if needed
        x_val = float(
            self.p.x[0] if hasattr(self.p.x, "ndim") and self.p.x.ndim > 0 else self.p.x
        )
        y_val = float(
            self.p.y[0] if hasattr(self.p.y, "ndim") and self.p.y.ndim > 0 else self.p.y
        )
        r_val = float(
            self.p.r[0] if hasattr(self.p.r, "ndim") and self.p.r.ndim > 0 else self.p.r
        )

        return circle(canvas, toX(x_val), toY(y_val), toP(r_val), color=self.c)
...
class Agent:
    ...
    def display(self, canvas, ball_x, ball_y):
        # Handle arrays by extracting the first element if needed
        bx = float(ball_x[0] if hasattr(ball_x, "ndim") and ball_x.ndim > 0 else ball_x)
        by = float(ball_y[0] if hasattr(ball_y, "ndim") and ball_y.ndim > 0 else ball_y)

        p = self.p
        # Extract scalar values from potentially array-typed position data
        x = float(p.x[0] if hasattr(p.x, "ndim") and p.x.ndim > 0 else p.x)
        y = float(p.y[0] if hasattr(p.y, "ndim") and p.y.ndim > 0 else p.y)
        r = float(p.r[0] if hasattr(p.r, "ndim") and p.r.ndim > 0 else p.r)

        # Convert direction to integer, handling array case
        direction_val = (
            p.direction[0]
            if hasattr(p.direction, "ndim") and p.direction.ndim > 0
            else p.direction
        )
        ...
        # Handle life as potentially being an array
        life_val = p.life[0] if hasattr(p.life, "ndim") and p.life.ndim > 0 else p.life
        num_lives = int(life_val)
